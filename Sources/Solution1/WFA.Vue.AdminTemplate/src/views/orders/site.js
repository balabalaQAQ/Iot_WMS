...
const uri = 'https://localhost:5001/api/WeatherForecast';   // Web API 的访问服务地址
let todos = [];                // 待办事宜的数据集合 
...
function addItem() {
    ...
    const addstockTextbox = document.getElementById('add-stock');
    ...
    const item = {
      //   Id: "4dc281ee-2081-4e07-be4d-89de00e60962",
         Name: addPriceTextbox.value.trim(),
         OrderNum: addNameTextbox.value.trim(),
...
function displayData(mumitems){//对显示的数据进行预处理
    ...
    for( var i=0;i< mumitems.data.length;i++){
      mumitems.data[i].status=statusitem[mumitems.data[i].status];
  
      //避免一些数据太长的影响
      if(mumitems.data[i].description.length>=10)
          mumitems.data[i].description=mumitems.data[i].description.substr(0,20)+"..."
      if(mumitems.data[i].name.length>=10)
          mumitems.data[i].name=mumitems.data[i].name.substr(0,10)+"..."
    }
    return mumitems.data;
  }